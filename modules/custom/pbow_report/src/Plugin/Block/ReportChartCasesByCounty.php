 ...
 *
 * @Block(
 *   id = "pbow_report_chart_cases_by_county",
 *   admin_label = @Translation("PBOW Report - Cases By County"),
 *   category = @Translation("Custom Blocks")
 * )
 */
class ReportChartCasesByCounty extends BlockBase {
  /**
   * {@inheritdoc}
   */
    ...
    $result = \Drupal::database()->query(
     "SELECT t.tid, t.name, COUNT(DISTINCT s.entity_id) AS cases, COUNT(DISTINCT u.uid) AS users
      FROM taxonomy_term_field_data t
        -- Cases
        LEFT JOIN node__field_county c ON t.tid = c.field_county_target_id
        LEFT JOIN node__field_case_status s ON c.entity_id = s.entity_id AND s.field_case_status_value BETWEEN 20 AND 50

        -- Users
        LEFT JOIN user__field_county uc ON t.tid = uc.field_county_target_id
    ...
    foreach ($result as $row) {
      $data['label'][] = str_replace(' County', '', $row->name);
      $data['cases'][] = $row->cases;
      $data['users'][] = $row->users;
    }

    $data['height'] = 50 + count($data['label']) * 20;

    return [
      '#theme' => 'pbow_report_chart_cases_by_county',
      '#data' => $data,
    ];
  }
